...
function downloadCSV() {
  // Usar los datos que ya tiene la gráfica en memoria
  const labels = realtimeChart.chart?.data.labels || [];
  const values = realtimeChart.chart?.data.datasets[0].data || [];

  if (labels.length === 0) {
    alert('No hay datos en la gráfica aún');
    return;
  }
  ...
  link.href = url;
  link.download = `datos_pv_${new Date().toISOString().slice(0,19).replace(/:/g,'-')}.csv`;
